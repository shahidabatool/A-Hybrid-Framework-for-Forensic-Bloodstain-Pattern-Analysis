...
def main():
    parser = argparse.ArgumentParser(description="Train Task 2 TSD ResNet-50 CBAM Model")
    ...
    parser.add_argument("--weights_path", type=str, required=True, help="Path to pre-trained bloodnet50_new.pth weights")
    ...
    # 1. Load custom BloodNet50 architecture (pre-trained on 5 classes)
    print(f"Loading pre-trained BloodNet weights from: {args.weights_path}")
    model = bloodnet50(num_classes=5)
    state_dict = torch.load(args.weights_path, map_location="cpu")
    model.load_state_dict(state_dict)

    # 2. Modify fully-connected layer for 3-class target
    ...
    for epoch in range(epochs):
        ...
        if epoch_val_acc > best_val_acc:
            ...
            best_model_path = os.path.join(args.output_dir, "best_tsd_model_resnet50.pth")
    ...
    history_df.to_csv(os.path.join(args.output_dir, "tsd_resnet50_training_history.csv"), index=False)
