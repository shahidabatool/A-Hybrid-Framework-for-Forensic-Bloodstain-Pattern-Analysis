...
LOCAL_MODEL_PATH = "/content/Models/model1_efficientnet_b0.pth"
DRIVE_MODEL_PATH = os.path.join(BASE_DIR, "Models", "model1_efficientnet_b0.pth")
...
def plot_confusion_matrix(cm, classes, title, save_path, cmap="Blues"):
    """Plots a gorgeous annotated confusion matrix using Seaborn."""
...
def plot_multiclass_roc(y_true, y_probs, classes, title, save_path):
    ...
    for i in range(n_classes):
        # Convert to binary task: current class (1) vs rest (0)
    ...
    # First, aggregate true binary targets and predictions across all classes
...
def evaluate_model():
    ...
    # 2. Initialize Model and Load State Dict
    ...
    # Save raw report
    raw_report_path = os.path.join(EVAL_DIR, "efficientnet_classification_report.txt")
    with open(raw_report_path, "w") as f:
        f.write("=================================================================\n")
        f.write("  BPA MULTI-TASK EFFICIENTNET-B0 — RAW TEST REPORT\n")
        f.write("=================================================================\n\n")
    ...
    bal_report_path = os.path.join(EVAL_DIR, "efficientnet_balanced_classification_report.txt")
    with open(bal_report_path, "w") as f:
        f.write("=================================================================\n")
        f.write("  BPA MULTI-TASK EFFICIENTNET-B0 — BALANCED TEST REPORT\n")
        f.write("=================================================================\n\n")
        f.write("--- Pattern Classification (4 Classes) ---\n")
    ...
    # 5. Generate Confusion Matrices & ROC Curves for Raw Test Set
    ...
    plot_confusion_matrix(cm_p, PATTERN_CLASSES,
                          "EfficientNet-B0: Pattern Confusion Matrix (Raw)",
                          os.path.join(EVAL_DIR, "efficientnet_pattern_confusion_matrix.png"),
                          cmap="Blues")
                          
    plot_confusion_matrix(cm_m, MECHANISM_CLASSES,
                          "EfficientNet-B0: Mechanism Confusion Matrix (Raw)",
                          os.path.join(EVAL_DIR, "efficientnet_mechanism_confusion_matrix.png"),
                          cmap="Greens")
                          
    plot_multiclass_roc(true_p_raw, probs_p_raw, PATTERN_CLASSES,
                        "EfficientNet-B0: Pattern ROC Curves (Raw)",
                        os.path.join(EVAL_DIR, "efficientnet_pattern_roc_auc.png"))
                        
    plot_multiclass_roc(true_m_raw, probs_m_raw, MECHANISM_CLASSES,
                        "EfficientNet-B0: Mechanism ROC Curves (Raw)",
                        os.path.join(EVAL_DIR, "efficientnet_mechanism_roc_auc.png"))
