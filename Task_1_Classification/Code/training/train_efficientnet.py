...
# Reading thousands of small files directly from mounted Google Drive FUSE is extremely slow and prone to FileNotFoundError.
# If the dataset is copied and unzipped to the local VM disk (/content/), we use that for a 100x training speedup!
LOCAL_DATA_DIRS = [
...
MODEL_SAVE_PATH = os.path.join(LOCAL_SAVE_DIR, "model1_efficientnet_b0.pth")
HISTORY_SAVE_PATH = os.path.join(LOCAL_SAVE_DIR, "model1_efficientnet_b0_history.json")
PLOT_SAVE_PATH = os.path.join(LOCAL_SAVE_DIR, "model1_efficientnet_b0_curves.png")
    ...
    def __init__(self, root_dir, transform=None):
        ...
        if len(self.image_paths) == 0:
            raise ValueError(
                ...
                "1. You have not run the data augmentation script ('augment_data.py') to populate the 'Data/Augmented/train' and 'Data/Augmented/test' directories.\n"
                "2. Your files are still syncing on Google Drive (wait a few minutes and re-run).\n"
                "3. Your class folders are empty or don't contain any (.jpg, .png, .jpeg) files."
            )
...
def save_history_and_plots(history, history_path, plot_path):
    ...
    plt.title("Multi-Task Loss Curve", fontsize=12, fontweight='bold')
    ...
    if 0 < phase1_epochs < len(epochs):
        plt.axvline(x=phase1_epochs + 0.5, color="gray", linestyle="--", label="Phase 2 Fine-tuning")
    plt.title("Pattern Classification Accuracy", fontsize=12, fontweight='bold')
    plt.xlabel("Epoch", fontsize=10)
    ...
    if 0 < phase1_epochs < len(epochs):
        plt.axvline(x=phase1_epochs + 0.5, color="gray", linestyle="--", label="Phase 2 Fine-tuning")
    plt.title("Mechanism Classification Accuracy", fontsize=12, fontweight='bold')
    plt.xlabel("Epoch", fontsize=10)
    plt.ylabel("Accuracy", fontsize=10)
    plt.legend()
    plt.grid(True, linestyle=":", alpha=0.6)
...
def train_model():
    ...
    # Use larger batch size on Colab GPU if possible
    ...
    model = BPAMultiTaskEfficientNet(num_pattern=4, num_mechanism=3).to(device)
    ...
    # Initialize dictionary to collect history for plotting/reproducibility
    history = {
        "epoch": [],
        "phase": [],
        "train_loss": [],
    ...
    # Phase 2: Unfreeze and fine-tune whole model with lower learning rate
