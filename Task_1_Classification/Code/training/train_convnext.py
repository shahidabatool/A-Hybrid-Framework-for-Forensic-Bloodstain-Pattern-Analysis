...
MODEL_SAVE_PATH = os.path.join(LOCAL_SAVE_DIR, "model3_convnext.pth")
HISTORY_SAVE_PATH = os.path.join(LOCAL_SAVE_DIR, "model3_convnext_history.json")
PLOT_SAVE_PATH = os.path.join(LOCAL_SAVE_DIR, "model3_convnext_curves.png")
...
# --- BPA Multi-Task Model Definition (Model 3: ConvNeXt-Tiny) ---
class BPAMultiTaskConvNeXt(nn.Module):
    def __init__(self, num_pattern=4, num_mechanism=3):
        super().__init__()
        # Load ConvNeXt-Tiny backbone for transfer learning
        self.backbone = timm.create_model('convnext_tiny', pretrained=True, num_classes=0)
        num_features = self.backbone.num_features
        
        # Branch 1: Pattern Type (4 classes: Gunshot, Impact, Passive, Transfer)
        self.pattern_head = nn.Linear(num_features, num_pattern)
        
        # Branch 2: Impact Force Mechanism (3 classes: Passive, Low Vel, Med/High Vel)
...
def save_history_and_plots(history, history_path, plot_path):
    ...
    plt.title("ConvNeXt Multi-Task Loss", fontsize=12, fontweight='bold')
    ...
    if 0 < phase1_epochs < len(epochs):
        plt.axvline(x=phase1_epochs + 0.5, color="gray", linestyle="--", label="Phase 2 Fine-tuning")
    plt.title("ConvNeXt Pattern Accuracy", fontsize=12, fontweight='bold')
    plt.xlabel("Epoch", fontsize=10)
    ...
    if 0 < phase1_epochs < len(epochs):
        plt.axvline(x=phase1_epochs + 0.5, color="gray", linestyle="--", label="Phase 2 Fine-tuning")
    plt.title("ConvNeXt Mechanism Accuracy", fontsize=12, fontweight='bold')
    plt.xlabel("Epoch", fontsize=10)
    plt.ylabel("Accuracy", fontsize=10)
    plt.legend()
    plt.grid(True, linestyle=":", alpha=0.6)
