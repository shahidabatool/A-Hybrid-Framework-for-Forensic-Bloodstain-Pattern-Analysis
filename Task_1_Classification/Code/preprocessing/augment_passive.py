...
CLEANED_DIR = os.path.join(BASE_DIR, "Data", "Cleaned", "Passive_Drip")
AUGMENTED_DIR = os.path.join(BASE_DIR, "Data", "Augmented")
TRAIN_DST = os.path.join(AUGMENTED_DIR, "train", "Passive_Drip")
TEST_DST = os.path.join(AUGMENTED_DIR, "test", "Passive_Drip")

TRAIN_TARGET = 1000
TEST_CAP_PER_PARENT = 30
...
aug_pipeline = A.Compose([
    ...
    A.GaussNoise(p=0.3),
    # Advanced shape warping and dropout to prevent shape-outline memorization
...
def main():
    ...
    if not images:
        print("[!] No cleaned Passive Drip images found.")
        return
        
    print(f"Passive Drip: splitting and augmenting {len(images)} clean images...")
    
    # Group by parent video
    parent_groups = {}
    ...
    print(f"  Passive Drip Completed: Saved {current_count} train images and {test_count} test images.")
