...
SRC_DIR = os.path.join(BASE_DIR, "Data", "Organized", "Gunshot")
DST_DIR = os.path.join(BASE_DIR, "Data", "Cleaned", "Gunshot")
...
def process_massive_image(img, filename, dst_dir):
    ...
    # HSV double hue red range for Gunshot
...
def detect_blood_and_crop(img):
    """For small images: standard color masking and crop to ROI on white background."""
    h, w = img.shape[:2]
    max_dim = 2048
    if max(h, w) > max_dim:
...
def main():
    ...
    print(f"Cleaning Gunshot: processing {len(images)} images...")
    ...
    print(f"Finished Gunshot: {saved_count}/{len(images)} images kept (some tiled into multiple).")
